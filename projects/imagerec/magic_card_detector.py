...
class MagicCardDetector:
            ...
            if ref_im[0] != '':
                try:
                    self.reference_images.append(
                        ReferenceImage(ref_im[0], None, self.clahe, imagehash.hex_to_hash(ref_im[1])))
                except:
                    print(ref_im)
                    exit()
        print('Done.')
    ...
    def read_and_adjust_reference_images(self, path):
        ...
        for i, filename in enumerate(filenames):
            ...
            #added
            x = ReferenceImage(img_name, img, self.clahe)
            with open('testout.txt', 'a+') as f:
                f.write(str(x.name)+';'+str(x.phash)+'\n')
            #end added
            #self.reference_images.append(
